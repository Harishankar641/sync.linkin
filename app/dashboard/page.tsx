...
export default async function DashboardPage() {
   ...
   * upgrade to an AI-scored job that runs once per (a,b) pair.
   */
    ...
    <AppShell>
      ...
      <>
        <PremiumWorkspace
          ...
          opportunityCount={ccOpportunities.length}
          networkCount={directory.length}
          networkPeople={directory.slice(0, 6).map((p: any) => ({
            id: p.id,
            name: String(p.display_name || p.email || "Professional"),
            avatarUrl: p.avatar_url ?? null,
            score: Number(p.connection_score ?? p.sync_score ?? p.match_score ?? 0),
            headline: p.portfolio_about ? String(p.portfolio_about).split("\n")[0] : null
          }))}
        />
        ...
        <div className="space-y-8">
          ...
          {/* Twin Lab — the original solo-testing network is a first-class
              destination. These are real seeded test personas backed by
              twin_profiles and the existing auto-reply API; no fake demo
              messages are rendered here. */}
          {twinComplete && (testPersonas?.length ?? 0) > 0 && (
            <section className="twin-lab-card" id="twin-lab">
              ...
              <div className="twin-lab-grid">
                {(testPersonas ?? []).map((p) => {
                  const display = p.display_name ?? p.email;
                  const [sampleName, ...sampleRoleParts] = display.split(" — ");
                  const sampleRole = sampleRoleParts.join(" — ") || "Professional twin";
                  return (
                    <form action={startTestConversation} key={p.id} className="twin-sample-card">
                    ...
                    {testConversations.map((c) => {
                      const otherId =
                        c.participant_a === user.id
                          ...
                          : c.participant_a;
                      const resumeName =
                        (testPersonas ?? []).find((p) => p.id === otherId)?.display_name ??
          ...
          <div id="discover"><DiscoverSearch directory={directory} /></div>
