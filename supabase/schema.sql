-- =========================================================================
-- SyncedIn schema — dependency-ordered edition
--
-- Regenerated from the original supabase/schema.sql so it runs top-to-bottom
-- on a BRAND NEW Supabase project without error. The original file grew
-- feature-by-feature over time and ended up with a few statements that
-- referenced tables not yet created at that point in the file (safe on an
-- existing DB where those tables already exist, but fatal on a fresh one).
--
-- Structure, top to bottom:
--   1. Extensions
--   2. CREATE TABLE — every table, dependency-sorted (a table always comes
--      after every other table its foreign keys point to)
--   3. ALTER TABLE ... ADD COLUMN / ADD|DROP CONSTRAINT — safe here because
--      every table from step 2 already exists
--   4. CREATE INDEX — safe here because every column (base or altered)
--      already exists
--   5. ENABLE ROW LEVEL SECURITY
--   6. CREATE POLICY (each preceded by its DROP POLICY IF EXISTS guard)
--   7. Functions + triggers
--
-- Same content as the original file, reordered only — every table, column,
-- index, and policy comment is preserved verbatim in place.
-- Idempotent: every statement uses IF NOT EXISTS / OR REPLACE / DROP-then-
-- CREATE, same as the original, so it's still safe to re-run.
-- =========================================================================
...
-- =========================================================================
-- 3. Columns added after initial release (ALTER TABLE ... ADD COLUMN etc.)
-- =========================================================================
