...
import { SyncMoment } from "./SyncMoment";
import { useAgreementRealtime } from "@/lib/hooks/useAgreementRealtime";
...
export function ChatUI({
  ...
  const runLoop = useCallback(async (opts?: { proposeNow?: boolean }) => {
    ...
    try {
      for (let i = 0; i < CLIENT_TURN_CAP; i++) {
        ...
        // (i.e. the model was overloaded even after server-side retries),
        // wait 8s and try once more silently before surfacing to the user.
  ...
  // Twin Lab live-test mode: when a sample persona conversation is newly
  // created, run the same production conversation API used by normal chats.
  // This replaces the old static "demo conversation" illusion with a real
  // model-backed exchange that writes messages into Supabase.
  useEffect(() => {
    if (!autoStart || !other.isTestPersona || initialMessages.length > 0) return;
    const key = `syncedin-live-test:${conversationId}`;
    if (window.sessionStorage.getItem(key) === "started") return;
    window.sessionStorage.setItem(key, "started");
    void runLoop();
  }, [autoStart, conversationId, initialMessages.length, other.isTestPersona, runLoop]);
  ...
  // Live agreement updates (#SyncMoment). Previously the ONLY way to learn
  // your counterpart accepted was to refresh — respond-agreement wrote the
  // row and sent an email/push, but an open tab just sat there. This
  // subscribes to that row so `otherResponse` updates the instant the
  // counterpart acts, no refresh needed.
  const handleCounterpartAgreementChange = useCallback(
    (row: { response: "accepted" | "rejected"; reason: string | null } | null) => {
      if (row === null) {
        // Counterpart's row (and everyone's) was cleared — a rejection
        // resets both sides server-side. Mirror that locally.
        setOtherResponse(null);
        return;
      }
      setOtherResponse({ response: row.response, reason: row.reason });
    },
    []
  );
  useAgreementRealtime(
    conversationId,
    selfUserId,
    handleCounterpartAgreementChange
  );

  // Fire the celebratory Sync Moment exactly once per user per
  // conversation, the moment `bothAccepted` becomes true — whether that
  // happens because *I* just clicked accept, or because the realtime
  // subscription above just told me the counterpart did. localStorage
  // (not a DB column) tracks "already shown" — deliberately lightweight
  // since this is a one-time UI flourish, not data that needs to sync
  // across devices.
  const [showSyncMoment, setShowSyncMoment] = useState(false);
  const syncMomentEvaluatedRef = useRef(false);
  useEffect(() => {
    if (!bothAccepted) return;
    if (syncMomentEvaluatedRef.current) return;
    syncMomentEvaluatedRef.current = true;
    const seenKey = `syncedin-sync-moment-seen:${conversationId}:${selfUserId}`;
    try {
      if (typeof window !== "undefined" && window.localStorage.getItem(seenKey)) {
        return; // already celebrated this one
      }
      window.localStorage.setItem(seenKey, "1");
    } catch {
      /* storage blocked (private browsing etc.) — show anyway, worst
         case is a repeat celebration on next visit, which is harmless */
    }
    setShowSyncMoment(true);
  }, [bothAccepted, conversationId, selfUserId]);
    ...
    >
      {showSyncMoment && (
        <SyncMoment
          ...
          mySummary={summaryResult?.summary ?? null}
          otherSummary={summaryResult?.counterpart_summary ?? null}
          excitementScore={summaryResult?.excitement_score ?? null}
          onClose={() => setShowSyncMoment(false)}
        />
      )}
      <style>{`
